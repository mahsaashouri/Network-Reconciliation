olsfc.pwfc2 <- function(x, h, breakpoints = c(5, 10, 15, 20), maxlag = 0, nolag = NULL, criterion = "AIC") {
  n <- length(x)
  ...
  fit_pwfc <- MASS::rlm(form_pwfc, data = modeldata_pwfc,  maxit = 2e+03)
  fit_olsfc <- MASS::rlm(form_olsfc, data = modeldata_olsfc,  maxit = 2e+03)
  ...
  if ("segment" %in% colnames(selected_data)) {
    ...
    for (i in seq_along(fc_olsfc)) {
      ...
      newdata_olsfc <- data.frame(trend1 = trend1[i], season = season_olsfc[i])
