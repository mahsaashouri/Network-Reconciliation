...
for(i in 1:2){
  for(j in 1:2){
    if(i!=j){
      nam <- paste('G1', paste(paste0('A', i),  paste0('A', j), sep = '::'), sep = '--') 
      names.node1 <- c(names.node1, nam)
    }
  }
}
names.node2 <- c()
for(i in 1:2){
  for(j in 1:2){
    if(i!=j){
      nam <- paste('G2', paste(paste0('B', i),  paste0('B', j), sep = '::'), sep = '--') 
      names.node2 <- c(names.node2, nam)
    }
  }
}
...
names.node.outer1 <- c()
for(i in 1:2){
  nam.o <- paste('G1', paste('other', paste0('A', i), sep = '::'), sep = '--')
  names.node.outer1 <- c(names.node.outer1, nam.o)
}
names.node.outer2 <- c()
for(i in 1:2){
  nam.o <- paste('G2', paste('other', paste0('B', i), sep = '::'), sep = '--')
  names.node.outer2 <- c(names.node.outer2, nam.o)
}


net.name <- c(names.node1, names.node.outer1, names.node2, names.node.outer2)
## rename the series
...
smatrix.net <- as.matrix(smatrix.group(data.network = data.network))
ngts.net <- ts(as.matrix(Aggreg.func.group(data.network)), frequency = 12, start = c(2010, 1))
