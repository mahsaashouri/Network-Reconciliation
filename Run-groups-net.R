...
library(hts)
library(SparseM)
source('smatrix-group.R')
source('ngts-group.R')
source('CG-shrink.R')
#source('olsfc.pwfc.AIC-BIC2.R')

data.network.event <- read_csv('SampleArt5Most.csv')[,-1]
data.network.event <- mutate(data.network.event, id = paste0("G1--", id))

data.network.art <- read_csv('SampleLang5Most.csv')[,-1]
data.network.art <- mutate(data.network.art, id = paste0("G2--", id))

## combine two groups of networks
data.network.all <- bind_rows(data.network.event, data.network.art)
...
smatrix.net <- smatrix.group(data.network)
ngts.net <- ts(as.matrix(Aggreg.func.group(data.network)), frequency = 12, start = c(2017, 11))


## training and test sets 6 months forecasts 
...
fc.BU.ols <- as.matrix(Aggreg.func.group(fc.ols.b.melt))
...
fc.BU.ets <- as.matrix(Aggreg.func.group(fc.ets.b.melt))
...
fc.BU.stl <- as.matrix(Aggreg.func.group(fc.stl.b.melt))
...
fc.BU.arima <- as.matrix(Aggreg.func.group(fc.arima.b.melt))
